...
public class Tpm2Serial {
	...
	public Tpm2Serial(String portName, int baud) throws IllegalArgumentException, NoSerialPortFoundException {
		...
		buffer = new byte[HEADERSIZE + 2000 * 3];
	...
	//System.arraycopy(data, 0, buffer, HEADERSIZE, data.length);
	//LOG.log(Level.INFO, "I written the rizzza");
	...
	private synchronized void writeSerialData(byte[] cmdfull) {
		...
		try {
		    ...
		    /*
		    byte[] testbuf = new byte[6010];
		    testbuf[0] = 'A';                                // Magic word          
		    testbuf[1] = 'd';
		    testbuf[2] = 'a';
		    testbuf[3] = (byte)((1) >> 8);      // LED count high byte                                                                                                                                                                           
		    testbuf[4] = (byte)((1) & 0xff);    // LED count low byte                                                                                                                                                                            
		    testbuf[5] = (byte)(testbuf[3] ^ testbuf[4] ^ 0x55); // Checksum    
		    for(int k=6; k< 2006; k++) {
			testbuf[k] = (byte)0xA0;
			testbuf[k+1] = (byte)0xFF;
			testbuf[k+2] = (byte)0x00;
			}*/
		    System.arraycopy(cmdfull, 0, buffer, HEADERSIZE, cmdfull.length);		    
		    port.output.write(buffer);

		    //LOG.log(Level.INFO, "Just wrote");
			//port.output.flush();
